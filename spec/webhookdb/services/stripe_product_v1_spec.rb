...
RSpec.describe Webhookdb::Services::StripeProductV1, :db do
  it_behaves_like "a service implementation", "stripe_product_v1" do
    let(:body) do
      JSON.parse(<<~J)
        {
  ...
  it_behaves_like "a service implementation that prevents overwriting new data with old", "stripe_product_v1" do
    let(:old_body) do
      JSON.parse(<<~J)
        {
          "id": "evt_1CiPtv2eZvKYlo2CcUZsDcO6",
  ...
  it_behaves_like "a service implementation that verifies backfill secrets" do
    let(:correct_creds_sint) do
      Webhookdb::Fixtures.service_integration.create(
        service_name: "stripe_product_v1",
        backfill_key: "bfkey",
      )
    end
  ...
  it_behaves_like "a service implementation that can backfill", "stripe_product_v1" do
    ...
    def stub_service_requests
      return [
        ...
        stub_request(:get, "https://api.stripe.com/v1/products?starting_after=prod_L34MlEyQQXRsnE").
            to_return(status: 200, body: page2_response, headers: {"Content-Type" => "application/json"}),
        stub_request(:get, "https://api.stripe.com/v1/products?starting_after=prod_L64MlEyQQXRsnE").
  ...
  describe "webhook validation" do
    let(:sint) { Webhookdb::Fixtures.service_integration.create(service_name: "stripe_product_v1") }
    ...
    describe "calculate_create_state_machine" do
      it "asks for webhook secret" do
        ...
        expect(sm).to have_attributes(
          ...
          complete: false,
          output: match("We've made an endpoint available for Stripe Product webhooks:"),
        )
      end
      ...
      it "confirms reciept of webhook secret, returns org database info" do
        ...
        expect(sm).to have_attributes(
          ...
          post_to_url: "",
          complete: true,
          output: match("Great! WebhookDB is now listening for Stripe Product webhooks."),
        )
      end
    end
    ...
    describe "calculate_backfill_state_machine" do
      ...
      it "asks for backfill key" do
        ...
        expect(sm).to have_attributes(
          ...
          complete: false,
          output: match("In order to backfill Stripe Products, we need an API key."),
        )
      end
      ...
      it "confirms reciept of backfill key, returns org database info" do
        ...
        expect(sm).to have_attributes(
          ...
          post_to_url: "",
          complete: true,
          output: match("Great! We are going to start backfilling your Stripe Products."),
        )
      end
    end
  end
