...
RSpec.describe Webhookdb::Services::IncreaseWireTransferV1, :db do
  it_behaves_like "a service implementation", "increase_wire_transfer_v1" do
    let(:body) do
      JSON.parse(<<~J)
        {
  ...
  it_behaves_like "a service implementation that prevents overwriting new data with old", "increase_wire_transfer_v1" do
    let(:old_body) do
      JSON.parse(<<~J)
        {
          "event_id": "transaction_event_123",
  ...
  it_behaves_like "a service implementation that can backfill", "increase_wire_transfer_v1" do
