...
RSpec.describe "Webhookdb::Organization", :db, :async do
  ...
  describe "plan_supports_integration?" do
    ...
    it "returns true if the organization has an active subscription" do
      ...
      expect(Webhookdb::Organization.plan_supports_integration?(sint)).to eq(true)
    end
    ...
    it "returns false if the organization has no subscription and sint is not in first two integrations" do
      expect(sint.organization.active_subscription?).to eq(false)
      ...
      expect(Webhookdb::Organization.plan_supports_integration?(twilio_sint)).to eq(true)
      expect(Webhookdb::Organization.plan_supports_integration?(shopify_sint)).to eq(false)
    end
  end
end
