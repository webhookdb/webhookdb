...
RSpec.describe Webhookdb::Services::SponsyStatusV1, :db do
  ...
  let(:sint) { fac.depending_on(publication_sint).create(service_name: "sponsy_status_v1").refresh }
  ...
  it_behaves_like "a service implementation", "sponsy_status_v1" do
    let(:body) do
      JSON.parse(<<~J)
        {
  ...
  it_behaves_like "a service implementation that prevents overwriting new data with old", "sponsy_status_v1" do
    let(:old_body) do
      JSON.parse(<<~J)
        {
          "id": "9782fb8c-5a69-4f01-b95c-f9b70b0412c8",
  ...
  it_behaves_like "a service implementation dependent on another", "sponsy_status_v1", "sponsy_publication_v1" do
    let(:no_dependencies_message) { "This integration requires Sponsy Publications to sync" }
  end

  it_behaves_like "a service implementation that can backfill", "sponsy_status_v1" do
    ...
    def stub_service_requests
      return [
        stub_request(:get, "#{root_url}/#{publication_id1}/status?afterCursor=&#{querystr}").
            with(headers: {"X-Api-Key" => /.*/}).
            to_return(status: 200, body: make_body(["2022-04-10", "2022-04-09"], "curs1a"), headers:),
        stub_request(:get, "#{root_url}/#{publication_id1}/status?afterCursor=curs1a&#{querystr}").
            to_return(status: 200, body: make_body(["2022-04-08", "2022-04-07"], "curs1b"), headers:),
        stub_request(:get, "#{root_url}/#{publication_id1}/status?afterCursor=curs1b&#{querystr}").
            to_return(status: 200, body: make_body([], nil), headers:),
        stub_request(:get, "#{root_url}/#{publication_id2}/status?afterCursor=&#{querystr}").
            with(headers: {"X-Api-Key" => /.*/}).
            to_return(status: 200, body: make_body(["2022-04-10", "2022-04-09"], "curs2a"), headers:),
        stub_request(:get, "#{root_url}/#{publication_id2}/status?afterCursor=curs2a&#{querystr}").
            to_return(status: 200, body: make_body(["2022-04-08", "2022-04-07"], "curs2b"), headers:),
        stub_request(:get, "#{root_url}/#{publication_id2}/status?afterCursor=curs2b&#{querystr}").
            to_return(status: 200, body: make_body([], nil), headers:),
      ]
    end

    def stub_service_request_error
      return stub_request(:get, "#{root_url}/#{publication_id1}/status?afterCursor=&#{querystr}").
          to_return(status: 503, body: "woah")
    end
  end

  it_behaves_like "a service implementation that can backfill incrementally", "sponsy_status_v1" do
    ...
    def stub_service_requests(partial:)
      if partial
        return [
          ...
          stub_request(:get, "#{root_url}/#{publication_id1}/status?afterCursor=cursor1a&#{querystr}").
          ...
          stub_request(:get, "#{root_url}/#{publication_id2}/status?afterCursor=cursor2a&#{querystr}").
              to_return(status: 200, body: make_body(["2022-09-01"], nil), headers:),
        ]
      end
      return [
        ...
        stub_request(:get, "#{root_url}/#{publication_id1}/status?afterCursor=cursor1b&#{querystr}").
        ...
        stub_request(:get, "#{root_url}/#{publication_id2}/status?afterCursor=cursor2b&#{querystr}").
            to_return(status: 200, body: make_body(["2022-08-31"], nil), headers:),
      ]
    end
  end
