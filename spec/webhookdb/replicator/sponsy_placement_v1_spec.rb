...
RSpec.describe Webhookdb::Services::SponsyPlacementV1, :db do
  ...
  it_behaves_like "a service implementation", "sponsy_placement_v1" do
    let(:body) do
      JSON.parse(<<~J)
        {
  ...
  it_behaves_like "a service implementation that prevents overwriting new data with old", "sponsy_placement_v1" do
    let(:old_body) do
      JSON.parse(<<~J)
        {
          "id": "9782fb8c-5a69-4f01-b95c-f9b70b0412c8",
  ...
  it_behaves_like "a service implementation dependent on another", "sponsy_placement_v1", "sponsy_publication_v1" do
    let(:no_dependencies_message) { "This integration requires Sponsy Publications to sync" }
  end

  it_behaves_like "a service implementation that can backfill", "sponsy_placement_v1" do
  ...
  it_behaves_like "a service implementation that can backfill incrementally", "sponsy_placement_v1" do
