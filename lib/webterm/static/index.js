(function () {
  ...
  if (window.whdbEnv) {
    window.whdbEnv.forEach(([key, value]) => env.set(key, value))
  ...
  window.wasmPrompt = async function (text, hidden, callback) {
    /**
     * Return appropriate prompts for text prompt. If it's a single-line prompt,
     * return just a 'p' prompt element. If it's multiline, add some 'p' elements,
     * with just the final 'p' as inline. This prevents the input from being
     * all the way to the right of the longest prompt line.
     */
    function promptElements(s) {
      const lines = s.split("\n");
      if (lines.length <= 1) {
        return ["", `<p class="inline-input-prompt">${text}</p>`];
      }
      const promptLine = lines.pop();
      const prefix = lines.join("\n");
      return [`<p>${prefix}</p>`, `<p class="inline-input-prompt">${promptLine}</p>`];
    }

    const [promptPrefix, promptElement] = promptElements(text);
    currentBlock.innerHTML += `${promptPrefix}
      <div class="inline-input-root">
      ${promptElement}
    ...
    currentInput.focus();
    function swapPrompt({value, isBreak}) {
      const inputEl = getInlineInput();
      value = value === undefined ? inputEl.value : value;
    ...
    currentInput.addEventListener("keyup", (e) => {
      if (isBreak(e)) {
        swapPrompt({isBreak: true});
      } else if (e.key === "Enter") {
        swapPrompt({isBreak: false});
      }
    });
    currentInput.addEventListener("paste", (e) => {
      // If we paste a string with newlines, submit that value.
      // Inputs will remove the newlines, which may be important,
      // and using a textarea isn't working right.
      const text  = e.clipboardData.getData("text") || '';
      const multiLines = (text.trimEnd().match(/\n/g) || []).length > 0;
      if (multiLines) {
        swapPrompt({value: text, isBreak: false})
      }
    });
  };

  log(
    "Welcome to the WebhookDB interactive terminal!",
    "",
    `View our guide by running '<span class="command">wh docs html</span>'`,
    `or visiting <a href="https://docs.webhookdb.com" target="_blank">https://docs.webhookdb.com</a>.`,
    "",
    "Run 'webhookdb auth login' to get started."
  );
